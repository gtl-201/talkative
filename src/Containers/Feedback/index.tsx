...
const Feedback: FC<any> = (props) => {
  ...
  const onSenFeedback = useCallback(() => {
    if (refContent.current === "") {
      ShowToast("error", language.LACK, language.LACK_CONTENT);
    } else {
      FeedbackServices.sendFeedback({
        content: refContent.current,
        type: valueType ? "Feedback" : "Error",
      });
      ShowToast("success", language.SUCCESS, language.FEEDBACK_RECEIVED);
    }
  }, [valueType]);
  ...
  return (
    ...
    >
        ...
        >
          <Text
            style={[
              ...
              {
                color: !valueType
                  ? color.TXT_BTN_INACTIVE
                  : color.TXT_BTN_ACTIVE,
              },
            ]}
          >
            {language.COMMENT}
      ...
      <TouchableOpacity style={styles.btnSendFeedback} onPress={onSenFeedback}>
