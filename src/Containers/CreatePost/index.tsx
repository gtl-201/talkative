...
const CreatePost: FC<any> = (props) => {
  ...
  const pickLocation = useCallback(() => {
    if (refLabel.current && refContent.current) {
      ...
      ShowToast("error", language.LACK, language.LACK_LABEL_OR_CONTENT);
    }
  }, []);
  ...
  const createPost = useCallback((coordinate: object) => {
    ...
    post.geohash = geohashForLocation([
      coordinate.latitude,
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity style={styles.btnPickLocation} onPress={pickLocation}>
