...
const EditContactUser: FC<any> = (props) => {
  ...
  const saveInfor = () => {
    ...
    ARRAY_SOCIAL_NETWORKS.map((item) => {
      if (refSocialNetwork.current[item])
        objectSocialNetwork = {
          ...objectSocialNetwork,
          [item]: refSocialNetwork.current[item],
        };
    });
  ...
  const renderSocialNetworks = (item: string) => {
    ...
    return (
      <View style={styles.viewContact} key={item}>
        <Icon
          name={ICON_SOCIAL_NETWORKS[item]}
          type={"FontAwesome"}
          style={styles.icon}
        ...
        <TextInput
          ...
          placeholder={
            item !== "phoneNumberPublic"
              ? item.charAt(0).toUpperCase() + item.slice(1)
              : language.PHONE_NUMBER
          }
          maxLength={100}
          keyboardType={TYPE_KEYBOARD_SOCIAL_NETWORKS[item]}
          onChangeText={(text) => (refSocialNetwork.current[item] = text)}
        >
          {userInfor[item]}
        </TextInput>
      </View>
  ...
  return (
    <View style={styles.container}>
      <Header
        title={language.EDIT_CONTACT_INFO}
        iconRight={"check"}
      ...
      <Text style={styles.txtNote}>
        <Text style={styles.txtNoteBold}>{language.NOTE + ": "}</Text>
        {language.EDIT_CONTACT_NOTE}
      </Text>
