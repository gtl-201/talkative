...
const Home: FC<any> = (props) => {
  ...
  const setStateMapReady = useCallback(
  ...
  const getLocation = async () => {
    ...
    Geolocation.getCurrentPosition(
      (position) => {
        ...
        PostDispatchs.getPostsByLocation([
  ...
  useEffect(() => {
    if (statusUpload == STATUS.SUCCESS || statusUpload.FAILED) {
      if (statusUpload == STATUS.SUCCESS) {
        ShowToast("success", language.SUCCESS, language.SHARED_YOUR_POST);
      } else {
        ShowToast("error", language.FAILED, language.FAILED_SHARED_YOUR_POST);
      }

      PostDispatchs.resetUploadPostStatus();
    } else {
      if (statusUpload == STATUS.LOADING) {
        ShowToast("info", language.UPLOADING, language.UPLOADING_YOUR_POST);
      }
    }
  }, [statusUpload]);
  ...
  const filterDataPosts = useCallback(
    (types: []) => {
      if (types.length === 0) {
        setDataPosts(posts);
      } else {
  ...
  return (
    ...
    >
      ...
      />
      <ListType
        filterDataPosts={filterDataPosts}
        color={color}
        language={language}
      />
      <Pressable
      ...
      <ListNewsPost
        ref={refListPost}
        animatedToRegion={animatedToRegion}
        color={color}
        posts={dataPosts}
        language={language}
      />
