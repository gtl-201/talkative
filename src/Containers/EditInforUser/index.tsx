...
const EditInforUser: FC<any> = (props) => {
  ...
  const [reverseName, setReverseName] = useState<boolean>(
    userInfor.reverseName
  );
  const [dob, setDob] = useState<Date>(userInfor.dob || new Date());
  const [open, setOpen] = useState<boolean>(false);
  const [gender, setGender] = useState<GENDER>(
    userInfor.gender || GENDER.CUSTOM
  );
  ...
  const saveInfor = () => {
    updateUserInfor({
      ...
      name: reverseName
        ? refFirstName.current + " " + refLastName.current
        : refLastName.current + " " + refFirstName.current,
  ...
  const changeFirstname = useCallback((text) => {
    ...
    refTextName.current?.setNativeProps({
      text: reverseName
        ? refFirstName.current + " " + refLastName.current
        : refLastName.current + " " + refFirstName.current,
    });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.viewContent}>
        ...
        >
          ...
          />
          <Icon type={"FontAwesome"} name={"refresh"} style={styles.iconTail} />
        </Pressable>
