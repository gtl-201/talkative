...
function* uploadPost(newPost: Post) {
  ...
  firestore()
    ...
    .then((querySnapshot) => {
      ...
      firestore()
        ...
        .doc("counter")
        .update({
          a_post: data.a_post + 1,
          [KEY_COUNTER_TYPE[newPost.type]]:
            data[KEY_COUNTER_TYPE[newPost.type]] + 1,
        })
        .then(() => {
  ...
  yield fetch(
    `https://api.opencagedata.com/geocode/v1/json?q=${newPost.latitude}+${newPost.longitude}&key=2026d938ce4f4512bf9aef254f93733b`
  )
      ...
      if (dataLocation.status.code === 200) {
        firestore()
          ...
          .then((querySnapshot) => {
            let data = querySnapshot.data();
            if (
              !data.c_country.includes(
                dataLocation.results[0].components["ISO_3166-1_alpha-3"]
              )
            ) {
              data.c_country.push(
                dataLocation.results[0].components["ISO_3166-1_alpha-3"]
