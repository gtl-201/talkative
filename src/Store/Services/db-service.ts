      ...
      await tx.executeSql(
        ...
        (tx, results) => {
          ...
          for (let i = 0; i < results.rows.length; i++) {
            ...
            items = {
              ...items,
              [row.key]: SETTING_OPTIONS[row.key][row.value],
            };
          }
          resolve(items);
        }
  ...
  DB.transaction(
    function (tx) {
      notificationItems.map((item) => {
        tx.executeSql(
          `INSERT INTO ${TABLE_NOTIFICATION} (id, title, body, time, data) VALUES (?,?,?,?,?)`,
