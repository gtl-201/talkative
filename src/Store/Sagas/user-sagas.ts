    ...
    if (response.error == null && response.downloadUrl != null) {
      ...
      if (action.needUpdateServer) {
        yield put({
          ...
          userInfor: { avatar: response.downloadUrl },
        });
      } else {
        yield put({
  ...
  try {
    ...
    if (response.error == null && response.downloadUrl != null) {
      if (action.needUpdateServer) {
        ...
        yield put({
          type: USER.UPDATE_USERINFOR,
          userInfor: { thumbnail: response.downloadUrl, posts: posts },
        });
      } else {
        yield put({
          type: USER.UPLOAD_THUMBNAIL_SUCCESS,
          downloadUrl: response.downloadUrl,
        });
      }
    } else {
