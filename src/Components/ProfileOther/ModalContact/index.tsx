...
const ModalContact = forwardRef(
  ({ color, userInfor, language }: ModalContactProps, ref) => {
    ...
    const copyContact = useCallback((contactInfor: string) => {
      if (contactInfor || contactInfor != "") {
        ...
        ShowToast("info", language.COPIED, contactInfor);
      }
    }, []);
      ...
      >
        <View style={styles.containerModal}>
          <View style={styles.container}>
            ...
            {isEmpty ? (
              <Text style={{ color: color.TXT_EMPTY }}>
                {language.NOTHING_FOR_YOU}
              </Text>
            ) : (
