...
const SearchBar: FC<Props> = (props) => {
  const { color, avatar, navigation, statusUpload, posts, nickname, language } =
    props;
  ...
  const createPostPress = useCallback(() => {
    if (posts.length >= 5) {
      ShowToast("info", language.MAX_5_POST, language.DELETE_TO_CREATE_POST, {
        onPress: () => {
          Toast.hide();
  ...
  return (
    <View style={styles.container}>
      ...
      {statusUpload == STATUS.LOADING ? (
        ...
        <TouchableOpacity style={styles.touchAdd} onPress={createPostPress}>
          <Icon
            type={"MaterialIcons"}
