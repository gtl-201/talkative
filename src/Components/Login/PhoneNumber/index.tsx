...
const PhoneNumber: FC<any> = forwardRef((props, ref) => {
  const { color, language, scrollTo, user, isUserReady, setUser, setLoading } =
    props;
  ...
  async function signInWithPhoneNumber() {
    const confirmation = await auth()
      .signInWithPhoneNumber(value + phoneNumber, true)
      .finally(() => {
        setLoading(false);
    ...
    onPressNext(index: number) {
      if (!checkPolicy) {
        ...
        if (phoneNumber.length < 5) {
          ShowToast("error", language.ERROR, language.INVALID_PHONE_NUMBER);
        } else {
          onPressSignin();
        }
      }
    ...
    <View style={styles.container}>
          ...
          style={[
            styles.viewInput,
            {
              ...
              borderBottomColor: open
                ? "#00000000"
                : color.BD_INPUT_PHONE_NUMBER,
            },
          ]}
        >
          <Pressable
            onPress={() => setOpen(true)}
            style={styles.pressCountryCode}
          >
      ...
      >
        <Icon
          ...
          name={
            checkPolicy ? "checkbox-marked-outline" : "checkbox-blank-outline"
          }
