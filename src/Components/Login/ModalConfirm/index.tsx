...
const InputCode: FC<any> = memo((props) => {
  ...
  const renderRowKeyBoard = useCallback(
  ...
  const renderButtonKeyBoard = useCallback(
    (item: any) => {
      return (
        <TouchableOpacity
          ...
          style={[
            styles.touchButtonKeyBoard,
            {
              backgroundColor:
                item.value == -2
                  ? color.BG_BTN_CLOSE
                  : item.value == -1 && !check
                  ? color.BG_BTN_INACTIVE
                  : color.BG_BTN_ACTIVE,
            },
          ]}
        >
          {item.value >= 0 ? (
            ...
            <Icon
              type={"MaterialIcons"}
              name={item.value == -1 ? "keyboard-arrow-right" : "clear"}
              size={moderateScale(item.value == -1 ? 33 : 25, 0.3)}
              color={"white"}
            />
          )}
        </TouchableOpacity>
  ...
  const pressButtonKeyBoard = useCallback(
    (value: number) => {
      if (value == -1) {
        ...
        if (currentInput != 6) {
          cellRefs.current[currentInput].setNativeProps({
            text: value.toString(),
          });

          if (values.length == 5 && !check) setCheck(true);
        ...
        {data.map((item) => (
          <TextInput
            ...
            style={[
              ...
              {
                borderColor:
                  currentInput == item.order
                    ? color.BD_INPUT_ACTIVE
                    : color.BD_INPUT_INACTIVE,
              },
            ]}
...
const ModalConfirm: FC<any> = forwardRef((props, ref) => {
  ...
  const decreaseNum = () => {
    if (countDown > -1)
      txtCountDown.current.setNativeProps({
