...
const Nickname: FC<any> = forwardRef(
  ({ color, language, scrollTo, setUserInfor }: NicknameProps, ref) => {
    ...
    const debouncedSearchNickname = useCallback(
      debounce(() => checkDuplicateNickname(), 500),
      []
    );
    ...
    useImperativeHandle(ref, () => ({
      onPressNext(index: number) {
        if (!invalidNickname) {
          ShowToast("error", language.ERROR, errorDetail);
        } else {
    ...
    const checkDuplicateNickname = async () => {
      await checkNickname(nickname.current.toLowerCase()).then(
        (resultCheck: boolean) => {
    ...
    const onChangeNickname = useCallback((text: string) => {
      ...
      if (nickname.current === "") {
        ...
        if (new RegExp(specialWords.join("|")).test(nickname.current)) {
          ...
          setInvalidNickname(false);
          setErrorDetail(
            language.NICKNAME_DONT_INCLUDE + specialWords.join(", ")
          );
        } else {
    ...
    return (
      <View style={styles.container}>
        ...
        {!invalidNickname ? (
          <Text style={styles.alertTxt}>{"* " + errorDetail}</Text>
        ) : null}
      </View>
    );
  }
